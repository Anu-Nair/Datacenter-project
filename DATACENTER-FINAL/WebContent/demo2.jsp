...
try {
  ...
  rs1=st1.executeQuery("select proj_name,count(*) from project p, employee e where p.dep_id = e.dep_id group by proj_id having count(*)=4 order by proj_name");
  //out.println("checked one_");
  rs2=st2.executeQuery("select emp_name from project p,employee e where proj_name = 'ProjectZ'and post='Clerk' and year(hire_date)>2000 and p.dep_id = e.dep_id");
  //out.println("checked two_");
  rs3=st3.executeQuery("select distinct post from employee  order by post desc");
  rs4=st4.executeQuery("select emp_id, emp_name, hire_date from employee where hire_date like '%-04-%' or hire_date like '%-05-%' order by hire_date desc;");
  rs5=st5.executeQuery("select emp_name, emp_id, salary from employee e, department d where dep_location = 'ABC Block' and e.dep_id = d.dep_id and comission IS NULL;");
  ...
  var s=<%=marks%>;alert("YOUR MARKS ARE = "+s); window.location.href ="_front1.html";</script>
  <%
...
marks=0;
  ...
  //while((e = e.getNextException()) != null)
     ...
     %> <script language="javascript"> var s=<%=txt%>;alert("SQL ERROR : " +txt); window.location.href="front1.html";</script> <%
  }
